...
function AddressesPage() {
  ...
  const [form, setForm] = useState({
    label: "บ้าน", recipient: "", phone: "", address_line: "",
  ...
  const add = async () => {
    if (!userId) return;
    setSaving(true);
    if (form.is_default) await supabase.from("user_addresses").update({ is_default: false }).eq("user_id", userId);
    ...
    <div className="space-y-6">
        ...
        {addresses.map((a) => (
          <div key={a.id} className="flex items-start justify-between gap-3 rounded-lg border bg-white p-4">
            <div className="min-w-0 flex-1">
              ...
              <div className="text-sm text-slate-600">{[a.address_line, a.district, a.province, a.postcode].filter(Boolean).join(" ")}</div>
            </div>
            <div className="flex flex-col gap-1">
              {!a.is_default && (
                <button onClick={() => makeDefault(a.id)} className="text-slate-400 hover:text-[color:var(--brand-navy)]" title="ตั้งเป็นค่าเริ่มต้น">
      ...
      <div className="rounded-lg border bg-white p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div><Label>ป้ายกำกับ</Label><Input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} maxLength={30} /></div>
          <div><Label>ผู้รับ</Label><Input value={form.recipient} onChange={(e) => setForm({ ...form, recipient: e.target.value })} maxLength={100} /></div>
          <div><Label>เบอร์โทร</Label><Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} maxLength={10} inputMode="numeric" /></div>
          ...
          <div className="sm:col-span-2"><Label>ที่อยู่</Label><Textarea rows={2} value={form.address_line} onChange={(e) => setForm({ ...form, address_line: e.target.value })} maxLength={200} /></div>
